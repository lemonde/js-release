...
function confirmCreation(registry, callback) {
  async.series([
    next => getChangelog(registry, (err, changelog) => {
...
function preRelease(registry, callback) {
  ...
  return exec(registry.staticConfig.preRelease, (err, stdout, stderr) => {
...
function createRelease(registry, callback) {
  services.createRelease(registry.dryRun, `v${registry.nextVersion}`, registry.changelog, (err, link) => {
    if (err) return callback(err);
...
function postRelease(registry, callback) {
  ...
  return exec(registry.staticConfig.postRelease, (err, stdout, stderr) => {
    if (stderr) console.log('stderr', stderr);
    callback(err, registry);
  });
}
